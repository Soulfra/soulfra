...
def _enrich_brand_data(brand: dict, db) -> dict:
    ...
    brand['colors_list'] = list(colors)  # Ensure it's a list, not dict_items

    # Legacy: also parse JSON if it exists
    if brand.get('colors'):
        try:
            parsed_colors = json.loads(brand['colors'])
            brand['colors_list'] = list(parsed_colors) if parsed_colors else list(colors)
        except:
            pass
