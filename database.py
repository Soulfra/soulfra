...
def init_db():
    ...
    # Shared responses - Shareable AI responses with QR codes and hashes
    conn.execute('''
    ...
    # Scan history - QR/UPC scanner validation tracking
    conn.execute('''
    ...
    # Response metrics - Time-series tracking for analytics
    conn.execute('''
        CREATE TABLE IF NOT EXISTS response_metrics (
            ...
            user_agent TEXT,
            ip_address TEXT,
            FOREIGN KEY (response_id) REFERENCES shared_responses(id)
        )
    ''')

    # User achievements - Battlenet-style avatar unlocking system
    conn.execute('''
    ...
    # Scan sessions - Replay system like Starcraft
    conn.execute('''
    ...
    # User stats - Aggregate for leaderboards
    conn.execute('''
        CREATE TABLE IF NOT EXISTS user_stats (
            user_id INTEGER PRIMARY KEY,
            total_scans INTEGER DEFAULT 0,
            total_responses_created INTEGER DEFAULT 0,
            total_views_earned INTEGER DEFAULT 0,
            total_xp INTEGER DEFAULT 0,
            scan_streak INTEGER DEFAULT 0,
            last_scan_date DATE,
            avatar_code TEXT DEFAULT 'default',
            FOREIGN KEY (user_id) REFERENCES users(id)
        )
    ''')
    ...
    # Auth providers - Link multiple auth methods to one user (GitHub, phone, email, wallet)
    conn.execute('''
    ...
    # Domain launches - Track waitlist launch progress for core 4 domains
    conn.execute('''
        CREATE TABLE IF NOT EXISTS domain_launches (
            domain_name TEXT PRIMARY KEY,
            base_launch_days INTEGER DEFAULT 90,
            current_signups INTEGER DEFAULT 0,
            instant_launch_threshold INTEGER DEFAULT 900,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            launched_at TIMESTAMP
        )
    ''')

    # Waitlist - Email signups for domain launches with letter assignment
    conn.execute('''
        CREATE TABLE IF NOT EXISTS waitlist (
            ...
            FOREIGN KEY (domain_name) REFERENCES domain_launches(domain_name),
            UNIQUE(email, domain_name),
            UNIQUE(domain_name, letter_code)
        )
    ''')

    # Custom domains - User's own domains in the "email web"
    conn.execute('''
    ...
    # Professional profiles - Licensed trade professionals (plumbers, electricians, HVAC, etc.)
    conn.execute('''
    ...
    # Voice tutorials - Educational content from professionals
    conn.execute('''
        CREATE TABLE IF NOT EXISTS tutorial (
            ...
            pseo_pages_count INTEGER DEFAULT 0,
    ...
    # pSEO landing pages - Auto-generated SEO pages (50+ per tutorial)
    conn.execute('''
        CREATE TABLE IF NOT EXISTS pseo_landing_page (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    # Leads - Customer inquiries from professional sites
    conn.execute('''
        CREATE TABLE IF NOT EXISTS lead (
            ...
            FOREIGN KEY (professional_id) REFERENCES professional_profile(id) ON DELETE CASCADE,
            FOREIGN KEY (tutorial_id) REFERENCES tutorial(id) ON DELETE SET NULL,
            FOREIGN KEY (landing_page_id) REFERENCES pseo_landing_page(id) ON DELETE SET NULL
        )
    ''')
    ...
    conn.execute('CREATE INDEX IF NOT EXISTS idx_auth_providers_user_id ON auth_providers(user_id)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_auth_providers_provider ON auth_providers(provider, provider_username)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_waitlist_domain ON waitlist(domain_name)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_waitlist_email ON waitlist(email)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_custom_domains_user ON custom_domains(user_id)')

    # Indexes for new tables
    conn.execute('CREATE INDEX IF NOT EXISTS idx_professional_profile_user_id ON professional_profile(user_id)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_professional_profile_subdomain ON professional_profile(subdomain)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_professional_profile_trade ON professional_profile(trade_category)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_tutorial_professional_id ON tutorial(professional_id)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_tutorial_status ON tutorial(status)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_pseo_page_professional_id ON pseo_landing_page(professional_id)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_pseo_page_tutorial_id ON pseo_landing_page(tutorial_id)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_pseo_page_slug ON pseo_landing_page(slug)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_lead_professional_id ON lead(professional_id)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_lead_status ON lead(status)')
