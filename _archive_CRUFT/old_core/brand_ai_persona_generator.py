...
def generate_brand_ai_persona(brand_slug: str, tier: str = 'free') -> Optional[Dict]:
    ...
    # Build config from individual columns (no config_json column exists)
    brand_config = {
        'name': brand['name'],
        'personality': brand.get('personality_tone', 'thoughtful'),
        'tone': brand.get('personality_tone', 'friendly'),
        'category': brand.get('category', ''),
        'tagline': brand.get('tagline', ''),
        'colors': {
            'primary': brand.get('color_primary', '#3498db'),
            'secondary': brand.get('color_secondary', '#2ecc71'),
            'accent': brand.get('color_accent', '#e74c3c')
        }
    }
...
def get_brand_ai_persona_config(brand_slug: str) -> Optional[Dict]:
    ...
    brand_config = {
        'name': result_dict['brand_name'],
        'personality': result_dict.get('personality_tone', 'thoughtful'),
        'tone': result_dict.get('personality_tone', 'friendly'),
        'category': result_dict.get('category', ''),
        'tagline': result_dict.get('tagline', ''),
        'colors': {
            'primary': result_dict.get('color_primary', '#3498db'),
            'secondary': result_dict.get('color_secondary', '#2ecc71'),
            'accent': result_dict.get('color_accent', '#e74c3c')
        }
    }
    ...
    return {
        ...
        'system_prompt': system_prompt,
        'personality': brand_config['personality'],
