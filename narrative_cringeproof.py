...
class NarrativeSession:
    ...
    def advance_chapter(self) -> Dict[str, Any]:
        ...
        if current_chapter >= total_chapters:
            ...
            # 🎉 Trigger quiz completion event hook
            # This sends welcome email, creates notification, and builds profile
            try:
                from event_hooks import on_quiz_completed
                on_quiz_completed(user_id=self.user_id, session_id=self.session_id, send_email=True)
            except Exception as e:
                print(f"⚠️  Event hook failed (but quiz still completed): {e}")
