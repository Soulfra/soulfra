...
def voice_bank():
    ...
    ad_mapper = VoiceAdMapper()
    seo_detector = VoiceSEODetector()
    ...
    for rec in recordings:
        ...
        # Analyze advertising opportunities for recordings with transcripts
        ad_opportunities = None
        seo_analysis = None
        if transcript:
            try:
                ad_opportunities = ad_mapper.analyze_recording(rec['id'])
                seo_analysis = seo_detector.analyze_recording(rec['id'])
            except Exception as e:
                print(f"Error analyzing recording {rec['id']}: {e}")
