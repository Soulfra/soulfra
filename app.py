...
from flask import Flask, render_template, render_template_string, request, redirect, url_for, flash, session, Response, jsonify, g, send_file, send_from_directory, abort
...
# Enable CORS for API endpoints (allows GitHub Pages to call Flask backend)
CORS(app, resources={
    r"/api/*": {
        "origins": [
...
# Add Jinja filters for Voice Studio
from datetime import datetime
...
# Register Suggestion Box routes for voice-first community feedback
...
# Register Pixel Compositor routes for visual layer blending
from composer_routes import register_composer_routes
register_composer_routes(app)

# Register Voice Dev Studio routes for voice-first development
from voice_dev_studio import register_voice_dev_routes
register_voice_dev_routes(app)

# Register CringeProof Feed routes for TikTok-style content
from cringe_feed import register_cringe_feed_routes
register_cringe_feed_routes(app)

# Register RSS Feed Aggregator for cross-site content stitching
from rss_feed_aggregator import register_rss_aggregator_routes
register_rss_aggregator_routes(app)

# Register Bot Comment routes for 85 IQ wholesome social proof
from bot_comment_routes import register_bot_routes
register_bot_routes(app)

# Register QR Authentication routes for device-based passwordless login
from qr_auth_routes import register_qr_auth_routes
register_qr_auth_routes(app)

# Register Leaderboard routes for reputation tracking and rankings
from leaderboard_routes import register_leaderboard_routes
register_leaderboard_routes(app)

# Register Profile routes - voice-verified skills
from profile_routes import register_profile_routes
register_profile_routes(app)

# Register Auth API routes - JSON auth for GitHub Pages
from auth_api import register_auth_api_routes
register_auth_api_routes(app)

# Register Ideas API routes - Extract ideas from voice transcriptions
from ideas_api import ideas_bp
app.register_blueprint(ideas_bp)

# Register Blog Generator API routes - Generate blog posts from ideas
from blog_generator_api import blog_generator_bp
app.register_blueprint(blog_generator_bp)

# Register GitHub Commit API routes - Auto-commit blog posts to GitHub
from github_commit_api import github_commit_bp
app.register_blueprint(github_commit_bp)

# Register Token Economy API routes - Earn/spend time tokens
from token_economy_api import token_economy_bp
app.register_blueprint(token_economy_bp)

# Register Draft Manager API routes - Timer-based draft system
from draft_manager_api import draft_manager_bp
app.register_blueprint(draft_manager_bp)

# Register Gamification API routes - Quiz, ratings, achievements, progress tracking
...
# Register Join routes for referral-based waitlist signup
...
# Register StPetePros routes for professional directory
from stpetepros_routes import register_stpetepros_routes
from auth_bridge import register_auth_bridge_routes
register_stpetepros_routes(app)
register_auth_bridge_routes(app)
...
# Temporarily disabled - DomainManager import error
...
# Register Simple Voice routes - dead simple voice recording (CORS-enabled for CringeProof)
from simple_voice_routes import register_simple_voice_routes
register_simple_voice_routes(app)

# Register Twilio VoIP integration - real phone number → voice memos (OPTIONAL)
try:
    from twilio_integration import register_twilio_routes
    register_twilio_routes(app)
except ImportError as e:
    print(f"⚠️  Twilio integration skipped: {e}")
    print("   Install with: pip install twilio")

# Register Public Voice Submission - NO login required, AI processing
from public_voice_submission import register_public_voice_routes
register_public_voice_routes(app)

# Register Snapshot Exporter - Encrypted database exports for GitHub Pages proof-of-work
from snapshot_exporter import register_snapshot_routes
register_snapshot_routes(app)

# Register Data Cleanup - Fix incomplete voice memos, quality reports
from data_cleanup import register_cleanup_routes
register_cleanup_routes(app)

# Register Universal Intake System - ONE endpoint for voice, PDF, screenshot, URL, text
from intake_routes import register_intake_routes
register_intake_routes(app)

# Register Account Claiming - Anonymous → Permanent account linking
from auth_claim_routes import register_claim_routes
register_claim_routes(app)

# Register Unified Multi-Modal Input - Voice, screenshot, drawing, text → training data
from unified_input import register_unified_routes
register_unified_routes(app)

# Register Widget Carousel - Embeddable rotating experiences with interaction logging
from widget_carousel import register_widget_carousel_routes
register_widget_carousel_routes(app)

# Register Database Admin - SharePoint-like interface for soulfra.db management
from database_admin import register_database_admin_routes
register_database_admin_routes(app)

# Register domain sandbox routes for AI-powered wordmap editing
from sandbox_routes import register_sandbox_routes
register_sandbox_routes(app)

# Register Voice CAPTCHA - Brand verification through voice recording
from voice_captcha import register_voice_captcha_routes
register_voice_captcha_routes(app)

# Register Customer Export - Clean customer data aggregation and export
from customer_export import register_customer_export_routes
register_customer_export_routes(app)

# Register Batch Workflows - Automated customer syncing across domains
from batch_workflows import register_batch_workflow_routes
register_batch_workflow_routes(app)

# Register Product Tracking - UPC/QR code product performance tracking
from product_tracking import register_product_tracking_routes
register_product_tracking_routes(app)

# Register Live Call-In Show routes - NPR-style radio show with voice reactions
from live_show_routes import live_show_bp
app.register_blueprint(live_show_bp)

# Register Ollama Connector routes - Multi-Ollama integration with Netflix-style notifications
from ollama_connector_routes import ollama_connector_bp
app.register_blueprint(ollama_connector_bp)
...
# Register auth routes (for signup/login across all domains)
try:
    ...
    try:
        create_users_table()
        print("✅ Auth system loaded on port 5001")
    except Exception as e:
        print(f"⚠️  Users table already exists or error: {e}")
except ImportError as e:
    print(f"⚠️  Auth routes not available: {e}")

# Register OSS checkout routes (Lightning Network + BTCPay Server)
try:
    from oss_checkout_routes import oss_checkout_bp
    from mvp_payments import init_mvp_payment_tables
    app.register_blueprint(oss_checkout_bp)
    try:
        init_mvp_payment_tables()
        print("✅ OSS Checkout system loaded (Lightning + BTCPay + Coinbase)")
    except Exception as e:
        print(f"⚠️  OSS payment tables error: {e}")
except ImportError as e:
    print(f"⚠️  Checkout routes not available: {e}")
...
try:
    ...
    try:
        ...
        print("   Takedown: POST /api/dmca/takedown")
        print("   Counter: POST /api/dmca/counter")
    except Exception as e:
        print(f"⚠️  OSP compliance tables error: {e}")
except ImportError as e:
    print(f"⚠️  OSP compliance routes not available: {e}")

# Register prepaid credits system (manual Zelle/Mercury payments)
try:
    from credits_routes import credits_bp, init_credits_tables
    app.register_blueprint(credits_bp)
    try:
        init_credits_tables()
        print("✅ Prepaid Credits system loaded (Zelle → Manual Credit)")
    except Exception as e:
        print(f"⚠️  Credits tables error: {e}")
except ImportError as e:
    print(f"⚠️  Credits routes not available: {e}")

# Register account recovery system (SOC2/GDPR compliant)
try:
    from recovery_routes import recovery_bp, init_recovery_tables
    app.register_blueprint(recovery_bp)
    try:
        init_recovery_tables()
        print("✅ Account Recovery system loaded (Password Reset + GDPR Export)")
    except Exception as e:
        print(f"⚠️  Recovery tables error: {e}")
except ImportError as e:
    print(f"⚠️  Recovery routes not available: {e}")

# Register URI routes (short links with OG images)
try:
    from uri_routes import uri_bp
    app.register_blueprint(uri_bp)
    print("✅ URI routes loaded (/v/id, /u/user, /t/token, /i/idea, /q/qr)")
except ImportError as e:
    print(f"⚠️  URI routes not available: {e}")

# Register workflow automation
try:
    from workflow_routes import register_workflow_routes
    register_workflow_routes(app)
    print("✅ Workflow automation loaded (content syndication)")
except ImportError as e:
    print(f"⚠️  Workflow routes not available: {e}")
...
try:
    ...
    print("✅ Universal workflow system loaded (/workflows, /pipelines)")
except ImportError as e:
    print(f"⚠️  Universal workflow routes not available: {e}")
...
try:
    ...
    print("✅ Workflow stats API loaded (/api/workflow-stats)")
except ImportError as e:
    print(f"⚠️  Workflow stats API not available: {e}")

# Register affiliate link tracker (referral system)
# Disabled to prevent duplicate blueprint registration (already registered above at line 173)
# try:
#     from affiliate_link_tracker import affiliate_tracker_bp
#     app.register_blueprint(affiliate_tracker_bp)
#     print("✅ Affiliate link tracker loaded (referral rewards)")
# except ImportError as e:
#     print(f"⚠️  Affiliate tracker not available: {e}")

# Register OAuth login (Google/GitHub/Apple)
try:
    from oauth_routes import oauth_bp, init_oauth_tables
    app.register_blueprint(oauth_bp)
    try:
        init_oauth_tables()
        print("✅ OAuth login loaded (Google/GitHub/Apple Sign In)")
    except Exception as e:
        print(f"⚠️  OAuth tables error: {e}")
except ImportError as e:
    print(f"⚠️  OAuth routes not available: {e}")

# Register Cal Mobile Interface - QR → Voice/Text → GitHub
try:
    from cal_mobile_routes import cal_mobile_bp
    app.register_blueprint(cal_mobile_bp)
    print("✅ Cal Mobile Interface loaded (QR → Voice/Text → GitHub)")
except ImportError as e:
    print(f"⚠️  Cal Mobile not available: {e}")

# Register Soulfra OAuth Provider - YOUR OAuth platform
try:
    from soulfra_oauth import soulfra_oauth_bp, init_oauth_tables as init_soulfra_oauth
    app.register_blueprint(soulfra_oauth_bp)
    try:
        init_soulfra_oauth()
        print("✅ Soulfra OAuth Provider loaded (Login with Soulfra)")
    except Exception as e:
        print(f"⚠️  Soulfra OAuth tables error: {e}")
except ImportError as e:
    print(f"⚠️  Soulfra OAuth not available: {e}")

# Register SOUL Marketplace - AI Workflow Marketplace
try:
    from soul_marketplace_routes import soul_marketplace_bp, init_soul_marketplace_tables
    app.register_blueprint(soul_marketplace_bp)
    try:
        init_soul_marketplace_tables()
        print("✅ SOUL Marketplace loaded (Upload workflows, link to moods)")
    except Exception as e:
        print(f"⚠️  SOUL Marketplace tables error: {e}")
except ImportError as e:
    print(f"⚠️  SOUL Marketplace not available: {e}")

# Register README Generator routes - Dynamic GitHub profile READMEs
try:
    from readme_generator import readme_gen_bp
    from badge_routes import badge_bp
    from embed_routes import embed_bp
    app.register_blueprint(readme_gen_bp)
    app.register_blueprint(badge_bp)
    app.register_blueprint(embed_bp)
    print("✅ GitHub README generator loaded (/api/readme/<slug>, /badge/<slug>/*.svg)")
    print("✅ Embeddable widgets loaded (/embed/<slug>/wordmap, /embed/<slug>/activity)")
except ImportError as e:
    print(f"⚠️  README generator not available: {e}")

# Register Device Fingerprinting routes
try:
    from device_routes import device_bp
    app.register_blueprint(device_bp)
    print("✅ Device fingerprinting loaded (/api/devices/<slug>, /api/devices/recording/<id>)")
except ImportError as e:
    print(f"⚠️  Device fingerprinting not available: {e}")

# Register Voice Scraper routes
try:
    from scraper_routes import scraper_bp
    app.register_blueprint(scraper_bp)
    print("✅ Voice scraper loaded (/api/news/<recording_id>, /news/<recording_id>)")
except ImportError as e:
    print(f"⚠️  Voice scraper not available: {e}")

# Register Recording Approval routes
try:
    from recording_approval_routes import approval_bp, create_approval_tables
    app.register_blueprint(approval_bp)
    create_approval_tables()
    print("✅ Recording approval loaded (/recordings/review, /api/recordings/pending)")
except ImportError as e:
    print(f"⚠️  Recording approval not available: {e}")

# Register Simple Authentication routes
try:
    from simple_auth_routes import auth_bp, init_auth_tables
    app.register_blueprint(auth_bp)
    init_auth_tables()
    print("✅ Simple auth loaded (/login, /signup, /recover, /logout)")
except ImportError as e:
    print(f"⚠️  Simple auth not available: {e}")

# Register Daily Worklog routes
try:
    from daily_worklog_routes import daily_bp
    app.register_blueprint(daily_bp)
    print("✅ Daily Worklog loaded (/daily, /api/daily/*)")
    print("   Auto-categorizes voice → work/ideas/personal/learning/goals")
except ImportError as e:
    print(f"⚠️  Daily worklog not available: {e}")

# Register Soulfra Master Auth (cross-domain unified login)
try:
    from soulfra_master_auth import master_auth_bp, init_master_auth_tables
    app.register_blueprint(master_auth_bp)
    init_master_auth_tables()
    print("✅ Soulfra Master Auth loaded (/api/master/*)")
    print("   Unified cross-domain authentication")
    print("   - /api/master/signup (create account across ALL domains)")
    print("   - /api/master/login (login works everywhere)")
    print("   - /api/master/verify (JWT token validation)")
except ImportError as e:
    print(f"⚠️  Master auth not available: {e}")

# Register Soulfra Pulse (central timer/calendar API)
try:
    from soulfra_pulse import pulse_bp
    app.register_blueprint(pulse_bp)
    print("✅ Soulfra Pulse loaded (/api/soulfra/pulse)")
    print("   Central hub for calendar, timers, recording routing")
    print("   - /api/soulfra/pulse (real-time feed)")
    print("   - /api/soulfra/pulse/emit (emit events)")
    print("   - /api/soulfra/recordings-for-domain/<domain> (routed recordings)")
    print("   - 'The Play' merge: WORK + IDEAS → both personas")
except ImportError as e:
    print(f"⚠️  Soulfra pulse not available: {e}")

# Register AI Identity Recovery (device-bound auth with AI verification)
# Temporarily disabled - duplicate blueprint name 'recovery'
# try:
#     from ai_identity_recovery import recovery_bp as ai_recovery_bp, init_recovery_tables
#     app.register_blueprint(ai_recovery_bp, name='ai_recovery')
#     init_recovery_tables()
#     print("✅ AI Identity Recovery loaded (/api/recovery/...)")
#     print("   Device-bound authentication with AI verification")
#     print("   - /api/recovery/request (start recovery process)")
#     print("   - /api/recovery/verify (verify answers + restore access)")
# except ImportError as e:
#     print(f"⚠️  AI Identity Recovery not available: {e}")

# Register Tier Progression API (tier checks, domain unlocking)
try:
    from tier_progression_routes import tier_api_bp
    app.register_blueprint(tier_api_bp)
    print("✅ Tier Progression API loaded (/api/tier/...)")
    print("   Tier-based domain unlocking and ownership tracking")
    print("   - /api/tier/check/<user_id> (check current tier)")
    print("   - /api/tier/unlock (unlock domain)")
    print("   - /api/tier/ownership/<user_id> (ownership breakdown)")
except ImportError as e:
    print(f"⚠️  Tier Progression API not available: {e}")
...
from brand_router import detect_brand_from_request, get_brand_config, check_geo_access

@app.before_request
def detect_active_brand():
    """
    Detect and set active brand for this request

    Checks:
    1. ?brand= query parameter (for local testing)
    2. Host header (for production domains)
    3. Defaults to 'stpetepros' for localhost
    4. Enforces geo-restrictions if configured

    Sets g.active_brand for use in templates and routes
    """
    brand_slug = detect_brand_from_request(request)
    g.active_brand = get_brand_config(brand_slug)

    # Check geo-restrictions
    allowed, reason = check_geo_access(request, g.active_brand)
    if not allowed:
            ...
            <body>
                <div class="box">
                    ...
                    <p style="font-size: 0.9rem; opacity: 0.7; margin-top: 2rem;">
                        {{ brand_name }} is only available in the {{ areas }} area.
                    </p>
                </div>
            </body>
            </html>
        ''', reason=reason, brand_name=g.active_brand.get('name'), areas=', '.join(g.active_brand.get('area_codes', []))), 403)
        return response
...
def waitlist_landing():
    """Waitlist landing page with competitive domain launches"""
    ...
    if hasattr(g, 'active_brand') and g.active_brand:
        if g.active_brand.get('slug') == 'stpetepros':
            ...
            for row in category_rows:
                category = row['category'].replace('-', '_')  # Convert 'pest-control' to 'pest_control'
...
def me_dashboard():
    """
    Personal Economy Dashboard + Domain Manager

    Shows user's wordmap economy:
    - Personal wordmap (authentic voice)
    - Domain ownership percentages
    - Content generation rewards
    - Domain connections (GitHub-style graph)
    - Subdomain management
    - Quick actions

    Query params:
    - email: Test mode with email (bypasses login)
    """
    # Check for test mode (email query param)
    test_email = request.args.get('email')

    if test_email:
        # Test mode: look up user by email
        from database import get_db
        db = get_db()
        user = db.execute('SELECT id, username, email FROM users WHERE email = ?', (test_email,)).fetchone()
        ...
        if not user:
            ...
            db.execute('''
                ...
                VALUES (?, ?, ?)
            ''', (test_email.split('@')[0], test_email, datetime.now(timezone.utc).isoformat()))
            db.commit()
            user = db.execute('SELECT id, username, email FROM users WHERE email = ?', (test_email,)).fetchone()
        ...
        # Not logged in and no test email
        flash('Please log in to access your dashboard', 'error')
        return redirect(url_for('signup'))

    from user_economy import get_economy_data
    from domain_api_routes import BRAND_MAP

    economy = get_economy_data(user_id)

    if 'error' in economy:
        # User doesn't exist yet, show minimal dashboard
        from database import get_db
        db = get_db()
        ...
        connections = []
        waitlist_entry = db.execute('SELECT domain_name, letter_code FROM waitlist WHERE email = ?', (user['email'],)).fetchone()

        if waitlist_entry:
            connections.append({
                'domain': f"{user['username']}.{waitlist_entry['domain_name']}.com",
                'brand': BRAND_MAP.get(waitlist_entry['domain_name'], 'hub'),
                'type': 'subdomain',
        ...
        # Render VM-style dashboard
        return render_template('me/vm_dashboard.html',
                             user=user,
                             connections=connections,
                             economy={'error': 'No economy data yet'})
...
def domain_page(domain_slug):
    ...
    db = get_db()
    domain_row = db.execute('''
        SELECT domain_slug, context_type, content
        FROM domain_contexts
        WHERE domain_slug = ? OR domain_slug LIKE ?
        LIMIT 1
    ''', (domain_slug, f'{domain_slug}%')).fetchone()
...
@app.route('/api/portfolio-scan')
def portfolio_scan():
    """
    Get complete portfolio scan data

    Returns:
        JSON with voice memos, blog posts, admin dashboards, QR codes, duplicates, etc.
    """
    try:
        network_map_path = Path(__file__).parent / 'network_map.json'

        if not network_map_path.exists():
            # Run scanner if no map exists
            import subprocess
            subprocess.run(['python3', 'portfolio_scanner.py'], cwd=Path(__file__).parent, check=True)
...
@app.route('/api/domains/<slug>/deploy', methods=['POST'])
def api_deploy_domain(slug):
    """
    Deploy domain locally using CringeProof template

    POST body: {"backend_url": "https://your-backend.com"} (optional)
    ...
    Returns:
    {
        ...
        "files_created": ["index.html", "config.js", ...]
    }
    """
    ...
    try:
        ...
        result = deployer.deploy_domain(slug, backend_url)
...
@app.route('/api/domains/<slug>/deploy-github', methods=['POST'])
def api_deploy_domain_github(slug):
    """
    Deploy domain to GitHub Pages

    POST body: {"create_repo": true} (optional)

    Returns:
    {
        "success": true,
        "slug": "cringeproof",
        "github_pages_url": "https://soulfra.github.io/cringeproof-site",
        "repo_name": "cringeproof-site"
    }
    """
    from domain_deployer import DomainDeployer
    ...
    try:
        data = request.get_json() or {}
        create_repo = data.get('create_repo', False)

        deployer = DomainDeployer()

        # First deploy locally if not already done
        local_result = deployer.deploy_domain(slug)
        if not local_result['success']:
            return jsonify(local_result), 400

        # Then push to GitHub
        github_result = deployer.deploy_to_github(slug, create_repo)

        if github_result['success']:
            return jsonify(github_result)
        else:
            return jsonify(github_result), 400
...
@app.route('/api/domains/<slug>/status', methods=['GET'])
def api_domain_deployment_status(slug):
    """
...
@app.route('/api/domains/<slug>/verify', methods=['POST'])
def api_verify_domain_ownership(slug):
    """
    Verify domain ownership via DNS TXT or file upload

    POST body: {"method": "dns"} or {"method": "file", "content": "..."}
    ...
    try:
        ...
        if method == 'dns':
            ...
            verified = manager.verify_domain_ownership(slug)
        elif method == 'file':
            # File upload verification (already handled in domain_manager.py)
            verified = manager.verify_domain_ownership(slug)
        else:
            return jsonify({
                'success': False,
                'error': 'Invalid verification method. Use "dns" or "file".'
            }), 400
        ...
        if verified:
            ...
            return jsonify({
                'success': False,
                'verified': False,
                'error': 'Verification failed. Check DNS TXT record or file upload.'
            }), 400
...
def admin_dashboard():
    ...
    # Get voice memo activity
    voice_memo_count = db.execute('SELECT COUNT(*) as count FROM voice_memos').fetchone()['count']
    recent_voice_memos = db.execute('''
        SELECT id, domain, transcript, audio_url, created_at
        FROM voice_memos
        ORDER BY created_at DESC
        LIMIT 10
    ''').fetchall()
    ...
    # Get API key stats
    api_key_count = db.execute('SELECT COUNT(*) as count FROM api_keys WHERE is_active = 1').fetchone()['count']
    api_keys_recent = db.execute('''
        SELECT api_key, github_username, tier, github_repos, github_followers, created_at
        FROM api_keys
        WHERE is_active = 1
        ORDER BY created_at DESC
        LIMIT 10
    ...
    return render_template(
        ...
        recent_voice_memos=[dict(m) for m in recent_voice_memos],
        api_key_count=api_key_count,
        api_keys_recent=[dict(k) for k in api_keys_recent],
...
@app.route('/dashboard')
def user_dashboard():
    """User dashboard - shows profile, tier, activity after GitHub login"""
    # Check if authenticated via GitHub
    if not session.get('github_authenticated'):
        return redirect('/github/login?redirect_to=/dashboard')
    ...
    is_admin = session.get('is_admin', False)
    has_starred = session.get('has_starred_repo', False)

    # Get recent activity from voice_memos table
    db = get_db()
    ...
    tier_info = {
        1: {'name': 'Explorer', 'desc': 'Basic access - explore the platform', 'color': '#999'},
        2: {'name': 'Builder', 'desc': 'File imports and collaboration', 'color': '#3498db'},
        3: {'name': 'Creator', 'desc': 'API access and advanced features', 'color': '#9b59b6'},
        4: {'name': 'Admin', 'desc': 'Full platform access + admin panel', 'color': '#e74c3c'}
    }

    current_tier = tier_info.get(tier, tier_info[1])
...
@app.route('/status-map')
def status_map():
    """
    Visual Status Map - Game-like debugging interface

    Shows deployment status with instant visual feedback:
    ✅ Working
    🔧 Needs setup
    ❌ Broken
    """
    ...
    try:
        db.execute('SELECT 1').fetchone()
        voice_suggestions = db.execute('SELECT COUNT(*) as c FROM voice_suggestions').fetchone()['c']
        voice_recordings = db.execute('SELECT COUNT(*) as c FROM simple_voice_recordings').fetchone()['c']
        status_checks.append({
            'category': 'Database',
            'name': 'SQLite Connection',
            'status': '✅',
            'details': f'{voice_suggestions} suggestions, {voice_recordings} recordings',
            'action': None
        })
    except Exception as e:
        status_checks.append({
            ...
            'status': '❌',
            'details': str(e),
            'action': 'Run: python3 init_db.py'
        })

    # 2. Voice Recording System
    voice_dir = Path('voice_recordings')
    if voice_dir.exists():
        webm_files = list(voice_dir.glob('*.webm'))
        status_checks.append({
            'category': 'Voice System',
            'name': 'Voice Recordings Folder',
            'status': '✅',
            'details': f'{len(webm_files)} WebM files',
            'action': None
        })
    else:
        status_checks.append({
            ...
            'action': 'Run: mkdir -p voice_recordings'
        })

    # 3. CringeProof Voting
    try:
        votes = db.execute('SELECT COUNT(*) as c FROM suggestion_votes').fetchone()['c']
        status_checks.append({
            'category': 'Voice System',
            'name': 'CringeProof Voting',
            'status': '✅',
            'details': f'{votes} votes cast',
            'action': None
        })
    except:
        status_checks.append({
            ...
            'action': 'Run: sqlite3 soulfra.db < CRINGEPROOF_VOTING_SCHEMA.sql'
        })
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            status_checks.append({
                ...
                'name': 'Ollama',
                'status': '✅',
                'details': f'{len(models)} models: {", ".join(models[:3])}',
                'action': None
            })
        else:
        ...
        status_checks.append({
            ...
            'action': 'Install: brew install ollama && ollama serve'
        })
    ...
    try:
        ...
        status_checks.append({
            ...
            'action': 'Install: pip3 install openai-whisper'
        })
    ...
    try:
        ...
        status_checks.append({
            ...
            'status': '🔧',
            'details': 'Not installed',
            'action': 'Install: brew install nginx (macOS) or apt install nginx (Linux)'
        })

    # 8. SSL/HTTPS
    ssl_dir = Path('/etc/letsencrypt/live')
    if ssl_dir.exists():
        domains = list(ssl_dir.glob('*'))
        status_checks.append({
            'category': 'Deployment',
            'name': 'SSL Certificate',
            'status': '✅',
            'details': f'{len(domains)} domains configured',
            'action': None
        })
    else:
        status_checks.append({
    ...
    # 9. Working Routes
    working_routes = [
        {'path': '/voice', 'name': 'Voice Recorder', 'status': '✅'},
        {'path': '/suggestion-box', 'name': 'Suggestion Box', 'status': '✅'},
        {'path': '/suggestion/<id>', 'name': 'Suggestion Detail', 'status': '✅'},
        {'path': '/@<brand>/suggestions', 'name': 'Brand Suggestions', 'status': '✅'},
        {'path': '/chat', 'name': 'Ollama Chat', 'status': '✅'},
        {'path': '/status-map', 'name': 'Status Map', 'status': '✅'},
    ]
    ...
    status_checks.append({
        ...
        'name': 'Project Files',
        'status': '⚠️',
        'details': f'{py_files} .py files, {md_files} .md docs',
        'action': 'Consider archiving to /archive/prototypes/ for simplicity'
    })
...
def cringeproof_game():
    """
    CringeProof Homepage - Live game hub with widget carousel hero

    Shows:
    - Widget carousel with live top plays/questions
    - Real-time stats (questions today, players online, ideas shared)
    - QR code for mobile quickstart
    ...
    # Questions answered today (narrative sessions)
    cursor = conn.execute('''
        SELECT COUNT(*) as count
    ...
    # Players online in last hour (widget interactions)
    cursor = conn.execute('''
        SELECT COUNT(DISTINCT user_id) as count
    ...
    ''')
    players_online = cursor.fetchone()['count']

    # Ideas shared today (voice recordings with transcriptions)
    cursor = conn.execute('''
        SELECT COUNT(*) as count
        FROM simple_voice_recordings
        WHERE DATE(created_at) = DATE('now')
    ...
    ''')
    ideas_shared = cursor.fetchone()['count']
    ...
    return render_template('cringeproof/index.html', stats=stats)
...
@app.route('/api/cringeproof/live-stats')
def cringeproof_live_stats():
    """
    API endpoint for live stats (called by homepage for updates)

    Returns JSON with current stats:
    - questions_today: Narrative sessions created today
    - players_online: Unique users in last hour
    - ideas_shared: Voice recordings with transcriptions today
    """
    import sqlite3
...
# ============================================================================
# VOICE ARCHIVE STATIC FILE SERVING - CringeProof Self-Hosting
# ============================================================================
@app.route('/wall.html')
def serve_wall():
    """CringeProof Wall - Anonymous voice memos with live wordmap"""
    return send_from_directory('../github-repos/voice-archive', 'wall.html')


@app.route('/record-simple.html')
def serve_record_simple():
    """CringeProof Voice Recorder"""
    return send_from_directory('../github-repos/voice-archive', 'record-simple.html')


@app.route('/index.html')
@app.route('/')
def serve_cringeproof_index():
    """CringeProof Homepage"""
    return send_from_directory('../github-repos/voice-archive', 'index.html')


@app.route('/login.html')
def serve_login():
    """CringeProof Login Page"""
    return send_from_directory('../github-repos/voice-archive', 'login.html')


@app.route('/mobile.html')
def serve_mobile():
    """CringeProof Mobile - Touch-Optimized Voice Recording"""
    return send_from_directory('../github-repos/voice-archive', 'mobile.html')


@app.route('/css/<path:filename>')
def serve_voice_css(filename):
    """Serve CSS files from voice-archive/css"""
    return send_from_directory('../github-repos/voice-archive/css', filename)


@app.route('/js/<path:filename>')
def serve_voice_js(filename):
    """Serve JS files from voice-archive/js"""
    return send_from_directory('../github-repos/voice-archive/js', filename)


# =============================================================================
# CRINGEPROOF INSTANT SHARING - Voice Idea Recording & Sharing
# =============================================================================

# Import dependencies for instant sharing
try:
    from whisper_transcriber import WhisperTranscriber
    WHISPER_AVAILABLE = True
except ImportError:
    WHISPER_AVAILABLE = False
    print("⚠️  Whisper not available for voice transcription")
...
try:
    ...
    print("⚠️  Ollama not available for idea extraction")
...
def generate_public_hash():
    """Generate short, URL-safe hash for public sharing (like YouTube IDs)"""
...
def save_voice_recording():
    """
    CringeProof Voice Recorder - Save voice recording with instant sharing
    ...
    Returns: {
        ...
        'recording_id': int,
        'idea_id': int,
        'public_url': str,    # <-- Instant shareable link
        'expires_at': str,
        'tier': str
    }
    """
    ...
    if idea_id:
        ...
        try:
            ...
            try:
                # Load template
                template_path = "voice-archive/templates/instant-share-template.html"
                with open(template_path, 'r') as f:
                    template = f.read()

                # Get audio URL (relative path for static hosting)
                audio_url = f"./audio.webm"

                # Copy audio file to static directory
                audio_dest = f"{share_dir}/audio.webm"
                with open(audio_dest, 'wb') as f:
                    f.write(audio_data)
                print(f"🔊 Audio copied to: {audio_dest}")

                # Format expiry warning
                expiry_warning = ''
                if expiry_time:
                    tier_emoji = {'anonymous': '⏰', 'free': '🎁', 'pro': '👑'}.get(tier, '⏰')
                    expiry_warning = f'<div class="expiry-warning"><strong>{tier_emoji} Expires:</strong> {expiry_time.strftime("%Y-%m-%d %H:%M UTC")}<br><small>Sign up to keep your ideas forever!</small></div>'

                # Audio player HTML
                audio_player = f'<audio controls class="audio-player"><source src="{audio_url}" type="audio/webm">Your browser doesn\'t support audio playback.</audio>'

                # Replace template variables
                static_html = template.replace('{{TITLE}}', html.escape(idea_title))
                static_html = static_html.replace('{{DESCRIPTION}}', html.escape(idea_text[:150]))
                static_html = static_html.replace('{{OG_DESCRIPTION}}', html.escape(idea_text[:200]))
                static_html = static_html.replace('{{SHARE_URL}}', share_url)
                static_html = static_html.replace('{{OG_IMAGE_URL}}', f"{share_url}/og.png")
                static_html = static_html.replace('{{AUTHOR}}', html.escape(idea_data.get('username', 'Anonymous') if 'username' in idea_data.keys() else 'Anonymous'))
                static_html = static_html.replace('{{TIER_EMOJI}}', {'anonymous': '⏰', 'free': '🎁', 'pro': '👑'}.get(tier, '⏰'))
                static_html = static_html.replace('{{TIER}}', tier.title())
                static_html = static_html.replace('{{SCORE}}', '85')  # Default score
                static_html = static_html.replace('{{CREATED_DATE}}', datetime.now().strftime('%Y-%m-%d'))
                static_html = static_html.replace('{{AUDIO_PLAYER}}', audio_player)
                static_html = static_html.replace('{{TEXT}}', html.escape(idea_text))
                static_html = static_html.replace('{{EXPIRY_WARNING}}', expiry_warning)
                static_html = static_html.replace('{{EXPIRED_OVERLAY}}', '')  # Not expired on creation

                # Save static HTML
                static_html_path = f"{share_dir}/index.html"
                with open(static_html_path, 'w') as f:
                    f.write(static_html)

                print(f"📄 Static HTML generated: {static_html_path}")
                print(f"🌍 Public URL: {share_url}")

            except Exception as e:
                print(f"⚠️  Static HTML generation failed: {e}")
                import traceback
                traceback.print_exc()
...
def public_idea_view(hash):
    """
    CringeProof Instant Sharing - PUBLIC view of voice idea (works like bore.pub)
    ...
    if not idea:
        ...
        return render_template_string("""
<!DOCTYPE html>
<html><head><title>Not Found - CringeProof</title>
<link rel="stylesheet" href="/css/soulfra.css"></head>
...
<a href="/" style="display: inline-block; margin-top: 2rem; padding: 1rem 2rem; background: #ff006e; color: white; text-decoration: none; font-weight: 700;">Go Home</a>
</div></body></html>
        """), 404
    ...
    audio_url = None
    if idea['recording_id']:
        recording = db.execute('SELECT id FROM simple_voice_recordings WHERE id = ?',
                              (idea['recording_id'],)).fetchone()
        if recording:
            audio_url = f"/api/simple-voice/play/{idea['recording_id']}"
    ...
    share_url = f"{request.scheme}://{request.host}/i/{hash}"
    og_image_url = f"{request.scheme}://{request.host}/i/{hash}/og.png"

    tier_emoji = {'anonymous': '⏰', 'free': '🎁', 'pro': '👑'}.get(idea['tier'], '⏰')
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>{html.escape(idea['title'])} - CringeProof</title>
<meta name="description" content="{html.escape(idea['text'][:150])}">

<!-- Open Graph / Facebook -->
<meta property="og:type" content="article">
<meta property="og:url" content="{share_url}">
<meta property="og:title" content="{html.escape(idea['title'])}">
<meta property="og:description" content="{html.escape(idea['text'][:200])}">
<meta property="og:image" content="{og_image_url}">

<!-- Twitter -->
<meta property="twitter:card" content="summary_large_image">
<meta property="twitter:url" content="{share_url}">
<meta property="twitter:title" content="{html.escape(idea['title'])}">
<meta property="twitter:description" content="{html.escape(idea['text'][:200])}">
...
@media (prefers-color-scheme: dark) {{
    ...
    [data-variant="brutalist"] .idea-container,
    :root:not([data-variant]) .idea-container {{
        background: var(--dark-bg);
        color: var(--dark-text);
        border-color: var(--dark-text);
    }}

    [data-variant="brutalist"] .idea-text,
    :root:not([data-variant]) .idea-text {{
        background: #2a2a2a;
        color: var(--dark-text);
    }}

    [data-variant="minimal"] .idea-container {{
        background: #111827;
        color: #f9fafb;
        border-color: #374151;
    }}

    [data-variant="minimal"] .idea-text {{
        background: #1f2937;
        color: #f3f4f6;
    }}

    [data-variant="gradient"] .idea-container {{
        filter: brightness(1.1);
    }}
}}

/* ========================================
   VARIANT A: BRUTALIST (Default)
   ======================================== */
[data-variant="brutalist"], :root:not([data-variant]) {{
...
[data-variant="brutalist"] .idea-title,
:root:not([data-variant]) .idea-title {{
...
[data-variant="minimal"] .idea-container {{
    background: var(--bg-color);
    border: var(--border-width) solid #cbd5e1;
    box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
    border-radius: 12px;
}}

[data-variant="minimal"] .idea-title {{
    font-family: 'Inter', system-ui, sans-serif;
    font-weight: 700;
    letter-spacing: -0.02em;
}}

[data-variant="minimal"] .idea-header {{
...
[data-variant="gradient"] .idea-title {{
...
[data-variant="gradient"] .idea-header {{
    border-bottom: 2px solid rgba(255, 255, 255, 0.2);
}}

[data-variant="gradient"] .idea-text {{
...
[data-variant="gradient"] .idea-meta {{
...
.cta-box {{
    background: linear-gradient(135deg, #ff006e 0%, #ffc6ff 100%);
    padding: 2rem;
...
<div class="container">
    <div class="idea-container">
        {'<div class="expired-overlay"><h2>⏰ This Link Has Expired</h2><p>Anonymous ideas expire after 24 hours.</p><p><strong>Want to keep your ideas forever?</strong></p><a href="/signup.html" class="cta-btn">Sign Up Free</a></div>' if is_expired else ''}

        <div class="idea-header">
            <h1 class="idea-title">{html.escape(idea['title'])}</h1>
            <div class="idea-meta">
                <span><strong>Author:</strong> {html.escape(idea['username'] or 'Anonymous')}</span>
                <span><strong>Tier:</strong> {tier_emoji} {html.escape(idea['tier']).title()}</span>
                <span><strong>Score:</strong> {idea['score']}/100</span>
                <span><strong>Created:</strong> {idea['created_at'][:10]}</span>
            </div>
        </div>

        {f'<audio controls class="audio-player"><source src="{audio_url}" type="audio/webm">Your browser doesn\'t support audio playback.</audio>' if audio_url and not is_expired else ''}

        <div class="idea-text">
            {html.escape(idea['text'])}
        </div>

        {f'<div class="expiry-warning"><strong>⏰ Expires:</strong> {idea["expiry_timestamp"][:16]}<br><small>Sign up to keep your ideas forever!</small></div>' if idea['expiry_timestamp'] and not is_expired else ''}

        <div class="cta-box">
            <h3 style="margin-bottom: 1rem; font-size: 1.5rem;">🎤 Record Your Own Idea</h3>
            <p style="margin-bottom: 1.5rem;">No writing, no editing, no performance anxiety.</p>
            <a href="/record-simple.html" class="cta-btn">Record Now</a>
        </div>
    </div>
</div>

<footer style="text-align: center; padding: 2rem; margin-top: 4rem; opacity: 0.8;">
    <p><strong>How This Works</strong></p>
    <p style="margin-top: 0.5rem;">Voice memo → Whisper transcription → Ollama AI extraction → Instant shareable link</p>
...
<!-- A/B Testing Variant Switcher -->
<div class="variant-switcher">
    <strong>🎨 Design:</strong><br>
    <button class="variant-btn" data-variant="brutalist">Brutalist</button>
    <button class="variant-btn" data-variant="minimal">Minimal</button>
    <button class="variant-btn" data-variant="gradient">Gradient</button>
</div>
...
(function() {{
    ...
    const savedVariant = localStorage.getItem('preferredVariant');
    const currentVariant = urlVariant || savedVariant || 'brutalist';

    document.documentElement.setAttribute('data-variant', currentVariant);

    if (urlVariant) {{
        localStorage.setItem('preferredVariant', urlVariant);
    }}
    ...
    document.querySelectorAll('.variant-btn').forEach(btn => {{
        ...
        btn.addEventListener('click', () => {{
            ...
            localStorage.setItem('preferredVariant', variant);
...
# =============================================================================
# END CRINGEPROOF INSTANT SHARING
# =============================================================================


# =============================================================================
# SIMPLE VOICE RECORDINGS - LIST API
# =============================================================================

@app.route('/api/simple-voice/list', methods=['GET'])
def list_voice_recordings():
    """
    Get list of voice recordings for current user or domain

    Query params:
    - domain: Filter by domain (e.g. 'cringeproof')
    ...
    Returns: [
        {
            ...
            'created_at': str,
            'file_size': int,
            'audio_url': str  # /api/simple-voice/audio/{id}
        }
    ]
    """
    domain = request.args.get('domain')
    ...
    db = get_db()
    query = 'SELECT id, filename, transcription, domain, created_at, file_size FROM simple_voice_recordings WHERE 1=1'
    params = []
    ...
    for rec in recordings:
        results.append({
            ...
            'audio_url': f'/api/simple-voice/audio/{rec["id"]}'
        })

    return jsonify(results)


@app.route('/api/simple-voice/audio/<int:recording_id>', methods=['GET'])
def get_voice_audio(recording_id):
    """
    Serve audio file for a voice recording

    Returns: audio/webm blob
    """
    db = get_db()
    recording = db.execute(
        'SELECT audio_data, filename FROM simple_voice_recordings WHERE id = ?',
        (recording_id,)
    ).fetchone()
...
# =============================================================================
# DOMAIN CLAIMING APIs with QR 2FA
# =============================================================================
...
@app.route('/api/claim/generate-qr', methods=['POST'])
def generate_qr_code():
    ...
    session[f'claim_secret_{username}'] = secret
    session[f'claim_username'] = username
...
def verify_claim_code():
    """Verify 2FA code and claim domain"""
    data = request.get_json() or {}
    username = data.get('username', '').lower().strip()
    code = data.get('code', '').strip()
    secret = data.get('secret', '').strip()

    if not all([username, code, secret]):
        return jsonify({'success': False, 'error': 'Missing required fields'}), 400
    ...
    try:
        ...
        if existing:
            return jsonify({'success': False, 'error': 'Username already claimed'}), 409
        ...
        if not user:
            db.execute(
                ...
                (username, f'{username}@cringeproof.com', '', datetime.utcnow())
            )
            db.commit()
            user = db.execute('SELECT id FROM users WHERE username = ?', (username,)).fetchone()
        ...
        # Claim domain
        db.execute("""
            INSERT INTO claimed_domains (username, user_id, totp_secret, domain, claimed_at)
            VALUES (?, ?, ?, ?, ?)
        """, (username, user['id'], secret, f'{username}.cringeproof.com', datetime.utcnow()))
        ...
        session.pop(f'claim_secret_{username}', None)
        session.pop(f'claim_username', None)

        return jsonify({
            'success': True,
            'username': username,
            'domain': f'{username}.cringeproof.com',
            'user_id': user['id']
        })

    except Exception as e:
        print(f"Error claiming domain: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
@app.route('/api/wall/feed', methods=['GET'])
def get_wall_feed():
    """Get The Wall feed with voice memos and wordmap"""
    domain = request.args.get('domain', 'cringeproof')
    ...
    try:
        ...
        recordings = db.execute("""
            SELECT
                ...
                domain
            FROM simple_voice_recordings
            WHERE domain = ? AND transcription IS NOT NULL
            ORDER BY created_at DESC
            LIMIT ?
        ...
        for rec in recordings:
            ...
            # Extract top words from transcription
            words = rec['transcription'].lower().split()
            word_counts = {}
            stopwords = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'is', 'are', 'was', 'were', 'be', 'been', 'being', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'should', 'could', 'can', 'may', 'might', 'must', 'i', 'you', 'he', 'she', 'it', 'we', 'they', 'this', 'that', 'these', 'those'}
        ...
        # Build wordmap from all words
        top_words = sorted(all_words.items(), key=lambda x: x[1], reverse=True)[:20]

        # Count unique contributors
        unique_users = set(rec['user_id'] for rec in recordings if rec['user_id'])

        return jsonify({
            'success': True,
            'memos': memos,
            'total_memos': len(recordings),
            'wordmap': {
                'top_words': top_words,
                'total_words': sum(all_words.values()),
                'contributor_count': len(unique_users)
            }
        })

    except Exception as e:
        print(f"Error loading wall feed: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/wall/comments/<int:recording_id>', methods=['GET'])
    ...
    try:
        ...
        for c in comments:
            ...
            formatted_comments.append({
                ...
                'time_ago': time_ago,
                'verification_badge': tier_badges.get(c['phone_verification_tier'], '')
            })
...
@app.route('/api/wall/comments', methods=['POST'])
def post_comment():
    """Post a comment on a recording (requires phone verification)"""
    data = request.get_json() or {}
    recording_id = data.get('recording_id')
    comment_text = data.get('comment_text', '').strip()
    phone = data.get('phone', '').strip()

    if not all([recording_id, comment_text, phone]):
        return jsonify({'success': False, 'error': 'Missing required fields'}), 400

    try:
        db = get_db()

        # Phone verification and rate limiting logic
        # For now, simple tier assignment based on session
        tier = 'phone_only'  # Default tier
        rate_limits = {
            'phone_only': 10,
            'phone_and_desktop': 50,
            'premium': 100
        }

        # Check rate limit (comments in last hour)
        hour_ago = datetime.utcnow() - timedelta(hours=1)
        recent_comments = db.execute("""
            SELECT COUNT(*) as count
            FROM recording_comments
            WHERE phone_hash = ? AND created_at > ?
        """, (phone, hour_ago.isoformat())).fetchone()

        if recent_comments['count'] >= rate_limits[tier]:
            return jsonify({
                'success': False,
                'error': f'Rate limit exceeded. {tier} tier allows {rate_limits[tier]} comments/hour'
            }), 429

        # Insert comment
        username = session.get('username', f'User{phone[-4:]}')

        db.execute("""
            INSERT INTO recording_comments
            (recording_id, comment_text, username, phone_hash, phone_verification_tier, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (recording_id, comment_text, username, phone, tier, datetime.utcnow().isoformat()))
        ...
        return jsonify({
            ...
            'verification_badge': tier_badges[tier],
            'rate_limit_total': rate_limits[tier],
            'rate_limit_remaining': rate_limits[tier] - recent_comments['count'] - 1
        })
...
# =============================================================================
# HUB CHATBOX ROUTER API
# =============================================================================

@app.route('/api/hub/message', methods=['POST'])
def api_hub_message():
    """
    Store hub chatbox conversation and routing decision

    POST body: {
        "conversation": [
            {"role": "user", "content": "I want to record voice ideas"},
            {"role": "bot", "content": "Perfect! Taking you to cringeproof.com..."}
        ],
        "routed_to": "cringeproof",
    ...
    if not conversation or not routed_to:
        return jsonify({'success': False, 'error': 'Missing conversation or routed_to'}), 400
    ...
    try:
        ...
        # Store in hub_messages table
        cursor = db.execute("""
            INSERT INTO hub_messages (content, source, metadata)
            VALUES (?, ?, ?)
        """, (
            conversation_json,
            'chatbox_router',
        ...
        db.execute("""
            INSERT INTO hub_routing_log (message_id, routed_to_domain, user_input, created_at)
            VALUES (?, ?, ?, ?)
        """, (
            message_id,
            routed_to,
            conversation[0].get('content', '') if conversation else '',
            timestamp
        ))
...
# =============================================================================
# END HUB CHATBOX ROUTER API
# =============================================================================

# =============================================================================
# CAL REASONING API - For legendary interventions from Arty interface
# =============================================================================

@app.route('/api/cal/reason', methods=['POST'])
def cal_reasoning():
    """
    Cal's deep reasoning endpoint - called when legendary responses triggered
    Uses Ollama with soul document for authentic Soulfra personality
    """
    from ollama_soul import ask_ollama_with_soul

    data = request.get_json()
    user_message = data.get('message', '')
    context = data.get('context', '')  # Arty's conversation history
    trigger = data.get('trigger', 'legendary')  # Why Cal was summoned

    # Build Cal's reasoning prompt
    cal_prompt = f"""You are Cal - the reluctant but brilliant problem-solver.

Arty (the enthusiastic one) has been talking with a user, but now YOU'RE being called in because this is important.
...
Your job: Cut through the excitement and give the REAL analysis. What's actually feasible? What are they missing? What's the smart next step?

Be grumpy, be honest, be useful. No cheerleading - just truth."""

    # Call Ollama with soul document
    response = ask_ollama_with_soul(cal_prompt, model='llama3.2:3b')

    return jsonify({
        'response': response,
        'cal_mood': 'grumpy but helpful',
        'timestamp': datetime.now().isoformat()
    })

# =============================================================================
# END CAL REASONING API
# =============================================================================
...
def publish_dashboard():
    """
    Publishing Dashboard - Simple interface for Cal voice → blog workflow

    Shows:
    - QR code to scan with phone
    - Recent blog posts
    - Publishing stats
    - Direct link to live Calriven blog
    """
    db = get_db()

    # Get Calriven brand_id
    calriven_brand = db.execute("SELECT id FROM brands WHERE slug = 'calriven'").fetchone()
    brand_id = calriven_brand['id'] if calriven_brand else 3

    # Get total posts for Calriven
    ...
    posts_this_month = db.execute('''
        SELECT COUNT(*) as count FROM posts
        WHERE brand_id = ? AND published_at >= ?
    ''', (brand_id, month_start)).fetchone()['count']
    ...
    posts_today = db.execute('''
        SELECT COUNT(*) as count FROM posts
        WHERE brand_id = ? AND published_at >= ?
    ''', (brand_id, today_start)).fetchone()['count']
...
def terms_of_service():
    ...
    Display Terms of Service & Privacy Policy (GDPR compliant)
    """
...
print("✅ Newsletter routes registered (/api/newsletter/*)")

# Waitlist routes
from waitlist_routes import waitlist_bp
app.register_blueprint(waitlist_bp)
print("✅ Waitlist routes registered (/api/waitlist/*)")

# Domain API routes
from domain_api_routes import domain_api_bp
app.register_blueprint(domain_api_bp)
print("✅ Domain API routes registered (/api/domains/*)")
...
if __name__ == '__main__':
    ...
    # Register Bonjour/mDNS service for auto-discovery (like AirDrop)
    try:
        ...
        service_name = "Soulfra Voice Recorder._http._tcp.local."
        ...
        info = ServiceInfo(
            ...
            server="soulfra.local.",
        )

        zeroconf.register_service(info)
        print("")
        print("📡 Bonjour/mDNS service registered")
        print(f"   Service: {service_name}")
        print(f"   Address: soulfra.local:5001")
        print(f"   Local IP: {local_ip}")
        print("   🎯 iPhone can now auto-discover this server!")
    except ImportError:
        print("")
        print("⚠️  Bonjour/mDNS disabled (zeroconf not installed)")
        print("   Install: pip3 install zeroconf")
    except Exception as e:
        print(f"⚠️  Bonjour/mDNS registration failed: {e}")

    # Check for SSL certificates (for HTTPS on iPhone)
    ssl_context = None

    # Try self-signed cert first (for CringeProof)
    cert_file = 'voice-archive/cert.pem'
    key_file = 'voice-archive/key.pem'

    # Fall back to mkcert certs if available
    ...
    if os.path.exists(cert_file) and os.path.exists(key_file):
        ...
        print("")
        print("🔒 HTTPS enabled with local SSL certificates")
        print(f"   Cert: {cert_file}")
        print("   Access from laptop: https://localhost:5001")
        print("   Access from iPhone: https://192.168.1.87:5001")
        print("   Access via Bonjour: https://soulfra.local:5001")
        print("   Microphone API: ✅ Works on iOS Safari")
        print("   CringeProof: ✅ CORS enabled for cringeproof.com")
    else:
        print("")
        print("⚠️  Running HTTP only (no SSL certs found)")
        print("   Microphone API: ❌ Blocked on iOS Safari")
        print("   Generate certs: mkcert localhost 192.168.1.87")
        print("   Or use file upload: /upload-voice")

    # Run with WebSocket support
    # socketio.run(app, host='0.0.0.0', debug=debug_mode, port=5001, allow_unsafe_werkzeug=True)
    app.run(host='0.0.0.0', debug=debug_mode, port=5001, ssl_context=ssl_context)
